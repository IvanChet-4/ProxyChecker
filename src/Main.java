...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            String address = "";
            int i;
            /* Читаем файл содержащий номера портов и ip */
            while ((i = fis.read()) != -1) {
                if (i == 13) continue;
                else if (i == 10) {
                    String ip = address.split("\t")[0];
                    int port = Integer.parseInt(address.split("\t")[1]);
                    /* Запускаем проверку и запись успешного результата */
    ...
    /* Проверяем подключение к url через прокси и в случае успешного подключеня записываем результат (ip, port) в файл goods_ip.txt */
